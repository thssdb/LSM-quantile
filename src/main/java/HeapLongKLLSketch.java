...
public class HeapLongKLLSketch extends KLLSketchForQuantile{
  ...
  protected int calcMaxMemoryNum(int maxMemoryByte){
    return Math.min(1<<20,maxMemoryByte/8-15);
  }
  ...
  protected void calcLevelMaxSize(int setLevel){ // set cntLevel.  cntLevel won't decrease
    ...
    for (int addK = 1 << 28; addK > 0; addK >>>= 1) { // find a new K to fit the memory limit.
      ...
      for (int i = 0; i < cntLevel; i++)
        need += Math.max(8, (int) Math.round(((newK + addK) * Math.pow(2.0 / 3, cntLevel - i - 1))));
...
//    System.out.println("T_T\t"+(R1-L1));
  ...
  @Override
  public void compact(){
    int compactLevel=cntLevel-1;double mxRate=0;
    for(int i=0;i<cntLevel;i++)
...
//        levelPos[i] = tmpL;
        if (i < oldCntLevel && i < another.cntLevel)
          cntPos += mergeSort(num, oldLevelPos[i], oldLevelPos[i + 1],
              another.num, another.levelPos[i], another.levelPos[i + 1], num, cntPos);
        else if (i < oldCntLevel)
          cntPos += mergeSort(num, oldLevelPos[i], oldLevelPos[i + 1], another.num, 0, 0, num, cntPos);
        else if (i < another.cntLevel)
          cntPos += mergeSort(num, 0, 0, another.num, another.levelPos[i], another.levelPos[i + 1], num, cntPos);
        levelPos[i] = tmpL;
      }
      ...
      for (int i = 0; i < cntLevel; i++) {
        int tmpL = numLen;
        if (i < oldCntLevel && i < another.cntLevel)
          numLen += mergeSort(oldNum, oldLevelPos[i], oldLevelPos[i + 1],
              another.num, another.levelPos[i], another.levelPos[i + 1], num, numLen);
        else if (i < oldCntLevel)
          numLen += mergeSort(oldNum, oldLevelPos[i], oldLevelPos[i + 1], another.num, 0, 0, num, numLen);
        else if (i < another.cntLevel)
          numLen += mergeSort(oldNum, 0, 0, another.num, another.levelPos[i], another.levelPos[i + 1], num, numLen);
        levelPos[i] = tmpL;
      }
...
//    System.out.print("\t\t\t\t[mergeWithTempSpace] others:");
    for(KLLSketchForQuantile another:otherList)  if(another!=null){
...
//    System.out.println("[mergeWithTempSpace]\totherNumLen:"+otherNumLen);
    if(getNumLen()+otherNumLen<=maxMemoryNum){
      ...
      for(int i=0;i<cntLevel;i++){
        ...
        for(KLLSketchForQuantile another:otherList)if(another!=null && i<another.cntLevel){
