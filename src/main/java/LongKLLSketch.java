...
public class LongKLLSketch extends KLLSketchForQuantile{
  ...
  public LongKLLSketch(int maxN, int maxMemoryByte, int maxSerializeByte) { // maxN=7000 for PAGE, 1.6e6 for CHUNK
    this.maxN = maxN;
    N = 0;
  ...
  protected void calcLevelMaxSize(int setLevel){ // set cntLevel.  cntLevel won't decrease
    ...
    for (int addK = 1 << 28; addK > 0; addK >>>= 1) {
      ...
      for (int i = 0; i < cntLevel; i++)
        need += Math.min(K << (maxLevel - i - 1), Math.max(8, (int) Math.round(((newK + addK) * Math.pow(2.0 / 3, maxLevel - i - 1)))));
...
//    show();
  ...
  public void showLevelMaxSize(){
    ...
    System.out.println("\t\tCOMPACT_SIZE:"+(13+(maxLevel)*(numLEN<256?1:2)+numLEN*8)+"\t//maxMemNum:"+maxMemoryNum+",maxSeriNum:"+maxSerializeNum+"\t//N:"+N);
    for(int i=0;i<cntLevel;i++)System.out.print("\t\t"+levelMaxSize[i]+"\t");System.out.println();
    System.out.println("-------------------------------------------------------");
  }
...
//    }
  ...
  public int serialize(OutputStream outputStream) throws IOException {// 15+1*?+8*?
    ...
    for(int i=levelPos[0];i<levelPos[maxLevel];i++)byteLen+=ReadWriteIOUtils.write(num[i],outputStream);
    return byteLen;
  }

  public LongKLLSketch(InputStream inputStream,int maxMemoryByte, int maxSerializeByte) throws IOException {
    ...
    for(int i=0,tmp=0;i<maxLevel;i++) {
      ...
      if(numLEN<256)
        tmp += UnsignedBytes.toInt(ReadWriteIOUtils.readByte(inputStream));
      else
        tmp += ReadWriteIOUtils.readShort(inputStream);
    }
    int actualLevel=maxLevel-1;
    while(levelPos[actualLevel]==levelPos[actualLevel+1])actualLevel--;
    calcLevelMaxSize(actualLevel+1);
    for(int i=0;i<numLEN;i++)num[maxMemoryNum-numLEN+i]=ReadWriteIOUtils.readLong(inputStream);
  }
  ...
  public LongKLLSketch(ByteBuffer byteBuffer, int maxMemoryByte, int maxSerializeByte) {
    ...
    for(int i=0,tmp=0;i<maxLevel;i++) {
      levelPos[i] = maxMemoryNum-(numLEN-tmp);
      if(numLEN<256)
        tmp += UnsignedBytes.toInt(ReadWriteIOUtils.readByte(byteBuffer));
      else
        tmp += ReadWriteIOUtils.readShort(byteBuffer);
    }
    int actualLevel=maxLevel-1;
    while(levelPos[actualLevel]==levelPos[actualLevel+1])actualLevel--;
    calcLevelMaxSize(actualLevel+1);
    for(int i=0;i<numLEN;i++)num[maxMemoryNum-numLEN+i]=ReadWriteIOUtils.readLong(byteBuffer);
  }
