...
//    Arrays.parallelSort(cluster,0,clusterNum, Comparator.comparingDouble(DoubleLongPair::getOne));
    sorted = true;
  }
...
//    System.out.println("\t\t add: "+(value));
  ...
  public void merge(List<TDigestForStatMerge> anotherList) {
    for(TDigestForStatMerge another:anotherList)
      merge(another);
  }
...
//    System.out.println("\t\tcompactBeforeSeri OVER\t");
  ...
  public TDigestForStatMerge(InputStream inputStream, int maxMemoryByte, int maxSerializeByte) throws IOException {
    this(maxMemoryByte);
